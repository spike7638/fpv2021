...
class affine_geom (α : out_param (Type u)) (β  : Type u) :=
...
def projective_geometry (α β : Type): Type := sorry 

def f : Type* → Type := sorry 
def g : Type* → Type := sorry

def foo (α β : Type) [affine_geom α β] : projective_geometry (f α) (g β) :=
sorry 


def my_fin4 : Type := fin 4

instance my_gp : group (my_fin4) := sorry 


#check ((*) : fin 4 → fin 4 → fin 4)
...
example (a : α) ( b : β) : meets a b :=
sorry 

example (b1 b2 : β) : parallel b1 b2 :=
sorry

lemma exists_s {P Q R : β} (hpq : ¬ parallel P Q) (hqr : ¬ parallel Q R) (hpr : ¬ parallel P R) (k n : β) : 
  ∃ s : β, ¬ parallel s k ∧ ¬ parallel k n :=
begin 
  by_contra h,
  push_neg at h,
  by_cases hpk : parallel P k,
  { have hQk : ¬ parallel Q k,
    { intro h',
      apply hpq,
      sorry },
...
noncomputable def nonpar_s {P Q R : β} (hpq : ¬ parallel P Q) (hqr : ¬ parallel Q R) (hpr : ¬ parallel P R) (k n : β) : 
  β :=
some (exists_s hpq hqr hpr k n)

lemma nonpar_s_spec {P Q R : β} (hpq : ¬ parallel P Q) (hqr : ¬ parallel Q R) (hpr : ¬ parallel P R) (k n : β) : 
  ¬ parallel (nonpar_s hpq hqr hpr k n) k :=
(some_spec (exists_s hpq hqr hpr k n)).left
...
lemma parallel_symmetric:
  ...
  begin
    ...
    by_cases k=n,
    {
      intro hp,
      rw h,
      apply parallel_prop_1, -- NOTE use of "G.parallel_prop_1" here, 
    },
    intro hpkn,
    have kNEn: k ≠ n := h,
    have ha : (k ≠ n) ∧ (k || n) := -- I cannot replace "affine_geom.parallel" here with G.parallel. Why??
    begin 
        exact ⟨h, hpkn⟩,
    end,
...
-- (a2              : ∀ P k n, ((meets P n ) ∧ (parallel n k)) ↔ (n = find_parallel P k))
    ...
    intro hp1,
    intro hp2,
    -- have hAnd: ((affine_geom.parallel q r) ∧ (affine_geom.parallel r s)) := and.intro hp1 hp2,
    -- revert hAnd,
...
--   have hk: ∃ kp:β  , affine_geom.parallel  kp (affine_geom.join Q R) :=
--   begin
--     -- let QR := affine_geom.join Q R,
--     -- let k := affine_geom.find_parallel P QR,
--     have hz: (affine_geom.meets P (affine_geom.find_parallel P (affine_geom.join Q R))) ∧ (affine_geom.parallel (affine_geom.find_parallel P (affine_geom.join Q R)) (affine_geom.join Q R)) := 
--     begin
...
begin
  ...
  have hmnl : ¬ affine_geom.parallel m l :=
  begin 
    intro hml,
    have hPQpQR : affine_geom.parallel PQ QR :=
    begin 
...
-- P Q, so S != P...
    begin
      ...
      have hmpq1: (affine_geom.meets R m)∧ (affine_geom.parallel m PQ) := begin
        apply (affine_geom.a2b R PQ m hm),
      end,
      have hmpqr: (affine_geom.parallel m PQ) := hmpq1.2,

-- show R is on m
      have hmPQ: ¬ (m = PQ) := 
      have hRma: affine_geom.meets R m  ∧ affine_geom.parallel m PQ:=
      begin
...
-- hence S <> P. Whew!
      ...
      begin
        -- R is on m
        have hRmz: (affine_geom.meets R m) ∧ (affine_geom.parallel m PQ):=
        begin
          apply (affine_geom.a2b R PQ m),
          exact hm,
        end,
...
-- suppose S = P; then m and PQ share a point. Contradiction.
-- hence S <> P. Whew!
      have hmPQz: affine_geom.meets R m ∧ affine_geom.parallel m PQ :=
      begin
        apply (affine_geom.a2b R PQ m),
        exact hm,
        -- (a2b              : ∀ P k n, (n = find_parallel P k) → ((meets P n ) ∧ (parallel n k)))
      end,
      have hmPQy: affine_geom.parallel m PQ := hmPQz.2,
-- hence m <> PQ and m || PQ (above)
-- so they share no points
      have hh: (m ≠ PQ ∧ affine_geom.parallel  m PQ) :=
      begin
        simp [hmPQ, hmPQy],
      end, 
...
-- But R DOES lie in QR, so S != R...
    begin
      ...
      have hlqr1: (affine_geom.meets P l) ∧ (affine_geom.parallel l QR) := begin
        apply (affine_geom.a2b P QR l hl), 
      end,

      have hlqrZ: (affine_geom.parallel l QR) := hlqr1.2,
      have hlQR: ¬ (l = QR) := 
        have hPla: affine_geom.meets P l  ∧ affine_geom.parallel l QR:=
        begin
...
-- hence S <> R. Whew!
      ...
      begin
        -- P is on l
        have hPlz: (affine_geom.meets P l) ∧ (affine_geom.parallel l QR):=
        begin
          apply (affine_geom.a2b P QR l),
          exact hl,
        end,
